...
import rospy
import tf
from brc_arm_roboclaw.msg import MotorPosition, EncoderValues
from brc_arm_roboclaw.srv import HomeArm, MoveClaw
...
class Joint:
    def __init__(self, name, roboclaw, address):
        ...
        if self.address > 0x87 or self.address < 0x80:
            ...
            return
        self.motorNum = rospy.get_param("/brc_arm_roboclaw/joints/" + name + "/motor_num")
        self.reduction = rospy.get_param("/brc_arm_roboclaw/joints/" + name + "/reduction")
        self.speed = rospy.get_param("/brc_arm_roboclaw/joints/" + name + "/speed")
        self.is_homed = False

        PID = rospy.get_param("/brc_arm_roboclaw/joints/" + name + "/PIDMM")
        try:
            if self.motorNum == 1:
...
class EEJoint:
    def __init__(self, name, roboclaw, address):
        ...
        if self.address > 0x87 or self.address < 0x80:
            ...
            rospy.signal_shutdown("Address out of range")
            return
        self.currentLimit = int(rospy.get_param("/brc_arm_roboclaw/joints/" + name + "/current_limit") * 100.0)
        self.motorNum = rospy.get_param("/brc_arm_roboclaw/joints/" + name + "/motor_num")
...
class Node:
    def __init__(self):
        ...
        rospy.init_node("brc_arm_roboclaw")
        rospy.on_shutdown(self.shutdown)

        roboclaws = rospy.get_param("/brc_arm_roboclaw/motor_controllers")
        joint_params = rospy.get_param("/brc_arm_roboclaw/joints")
