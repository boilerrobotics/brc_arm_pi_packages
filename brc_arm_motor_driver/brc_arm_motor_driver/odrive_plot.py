...
try:
    ...
    if(input("Trap control (y/n):") == "y"):
        odrv_axis.controller.config.input_mode = InputMode.TRAP_TRAJ
        ...
        odrv_axis.controller.config.input_mode = InputMode.PASSTHROUGH
    print("Set Values.")
    input("Press Enter to Start Motor")
    odrv_axis.requested_state = AxisState.CLOSED_LOOP_CONTROL
    position = 0
    increment = 0
